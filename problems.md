...
Most of you probably think that I'm talking about the guest GPU for my virtual machine, but I'm not. This problem is
about my host GPU since my Vega 56 seems quite good for a Windows VM. So at the beginning I wanted to use a GT 710
but it only has one DVI and one HDMI port and my monitors do not support DVI. Additionally the graphics card has to
handle a 3440x1440p @75Hz ultrawide display but the HDMI port of the GT 710 does not offer the needed bandwidth.  
The second card I wanted to use was an older R9 290. Unfortunately that card didn't work at all. Neither Windows nor
the BIOS recognized the card so I decided to buy a new one. Since I am using Linux as my host OS, I bought an AMD card.
To be precise, it was an RX 550 with one HDMI, DP and DVI port. That card should be enough for my purpose, especially
because I won't need an extremely well performing card for Linux.

### Solution

I bought an RX550 and all of my problems were gone.

## B550 board and its IOMMU groups

### Problem

**I**nput-**O**utput **M**emory **M**anagement **U**nit or in short IOMMU connects devices that are able to access the
memory directly (DMA-capable) at an address of the main memory. I really don't want to get into details and therefore
I will only explain all technical background information concerning this problem. Nonetheless, the problem has its origin
in the B550 chipset of my mainboard. Unfortunately, the B550 chipset puts all lanes connected to it into one IOMMU group.
In my case it's two PCIe slots, the ethernet controller an NVMe slot and some more stuff. But when passing a device, e.g.
my GPU, to a VM I have to pass everything of exact that IOMMU group also to my VM. Meaning, that if I wanted to use my guest
GPU in the second or third PCIe slot, I would have to pass everything else too. Luckily, the first PCIe slot is directly
connected to the CPU. That's the reason why I put my Vega into the first slot. It seemes to work for now but who knows what
problems I will get into within the next weeks.

### Solution

I enabled CSM in my BIOS settings and ended up with more IOMMU groups than before. You can check my IOMMU groups
in [this document](passthrough/pop_os/first_try.md).

## USB devices and audio

### Problem

Of course, I also need to pass some USB devices like a keyboard or a mouse and my audio to use a microphone or some
headphones. Passing through a keyboard and a mouse is quite easy but at first, I had some problems with my controller
and I really need to check how I can use my normal audio devices within the VM.

### Solution

In order to use unconventional input devices, I recommend passing a whole USB controller. It also allows you to
pass audio devices that are connected via USB ports. You can get more information [here](passthrough/extra/good_to_know.md#USB-Controllers).
...
BIOS of the card, can be passed through multiple times without rebooting if you do some workarounds. My Vega 56 suffers from
that bug and before I'm not able to buy a new GPU, I have to check if I can find a fix for it.
...
**Update 2022-12-17:** While using a 6700XT for about half a year, I wasn't able to find a solution to its "reset-bug". I
bought it on one hand to gain more performance and on the other to fix the reset-bug of my Vega 56. Navi 2 was known for
not having the common reset-bug and therefore I had no worries when I bought the PowerColor Red Devil 6700XT. However, it
turned out that some models of these Navi cards still don't reset the right way after restarting or shutting down the guest.
I then decided to buy another Navi 2 card, a ASUS TUF 6950XT. As I expected, I couldn't reproduce the bug on this one. I
only swapped GPUs and adjusted the device IDs in my GRUB-config and thus I can now restart my guest as often as I want.
...
**Update:** I wanted to use my controller multiple times but it always got reset. Blacklisting the ``xpad`` driver only delayed the resetting by some
time. If you really want to use devices that cannot be passed normally, I recommend passing a whole USB controller. You can find more about it
[here](passthrough/extra/good_to_know.md)
